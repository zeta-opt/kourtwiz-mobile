...
const GroupInfoScreen: React.FC = () => {
  ...
  // Resolve groupId properly
  const currentGroupId = typeof id === 'string' ? id : Array.isArray(id) ? id[0] : undefined;

  // Determine admin safely using userId
  const isCurrentUserAdmin = group?.members?.find((m: any) => m?.userId === userId)?.admin;

  const membersCount = group?.members?.length ?? 0;
  // use undefined instead of 'auto'
  ...
  useEffect(() => {
    const loadFavorites = async () => {
      if (!userId) return setFavoriteGroups([]);
      try {
        const key = `favoriteGroups_${userId}`;
        const raw = await AsyncStorage.getItem(key);
        const storedFavorites: string[] = raw ? JSON.parse(raw) : [];
        setFavoriteGroups(Array.isArray(storedFavorites) ? storedFavorites : []);
      } catch (err) {
  ...
  // Improved toggleFavorite: persist, put newly-favored on top, reorder local groups if present, emit event
  const toggleFavorite = async (groupIdParam?: string) => {
    const groupId = groupIdParam ?? currentGroupId;
    if (!groupId || !userId) {
      Alert.alert('Error', 'Invalid group or user ID.');
      return;
    }
    ...
    if (favoriteGroups.includes(groupId)) {
      ...
      // adding to favorites → place this new favorite at index 0
      updated = [groupId, ...favoriteGroups.filter((id) => id !== groupId)];
    }
  ...
  const handleToggleAdmin = async () => {
    if (!currentGroupId || !selectedMember || !userId) {
  ...
  const handleRemove = () => {
    ...
    if (!selectedMember?.phoneNumber || !userId) {
      ...
      <LinearGradient colors={['#E0F7FA', '#FFFFFF']} style={{ flex: 1, padding: 10 }}>
        ...
        <View style={[styles.membersListContainer, { maxHeight: maxListHeight }]}>
          <FlatList
            ...
            keyExtractor={(item, index) => (item.userId || item.id || String(index))}
            scrollEnabled={true}
        ...
        <View style={styles.actionContainer}>
          <TouchableOpacity
            ...
            onPress={() => toggleFavorite(currentGroupId)}
          >
            <Text style={styles.actionText}>
              {favoriteGroups.includes(currentGroupId ?? '') ? 'Remove from Favorites' : 'Add to Favorites'}
            </Text>
          </TouchableOpacity>
          ...
          {userId === (group?.createdByUserId ?? group?.createdBy) && (
